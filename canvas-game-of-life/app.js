...
class Cell {
    ...
    calcNeighbours(gridArr) {
        this.neighbourTopLeft = gridArr[this.index - this.grid.columns - 1]
            ? gridArr[this.index - this.grid.columns - 1].currentState * this.neighbourTopLeftWeight
            : 0;
        this.neighbourTopMid = gridArr[this.index - this.grid.columns]
            ? gridArr[this.index - this.grid.columns].currentState * this.neighbourTopMidWeight
            : 0;
        ...
        this.neighbourBottomMid = gridArr[this.index + this.grid.columns]
            ? gridArr[this.index + this.grid.columns].currentState * this.neighbourBottomMidWeight
            : 0;
    ...
    draw(prevState) {
        ...
        const sat =
            this.currentState === 0 && prevState === 1 ? this.saturation * 0.7 : this.saturation;
        ...
        if (
            ...
            (this.index === grid.clickIndex && (this.currentState != 0 || prevState != 0))
        ) {
            this.ctx.fill();
        }
...
fps.addEventListener('change', (e) => {
    const value =
        parseInt(e.target.value) <= grid.minTimeInterval || !parseInt(e.target.value)
            ? grid.minTimeInterval
...
function mainApp() {
    ...
    zoomLabel.innerText = window.innerWidth > 800 ? `Zoom: ${zoom.value}x` : `${zoom.value}x`;
}
