...
const [navAbout, navProjects, navContact] =
...
function initProjects() {
  projectsListArr.forEach((project) => {
    ...
    projectsListImageTextWrapperEl.classList.add(
    ...
    projectsListTextSubtitleEl.classList.add(
...
function initClouds(count) {
  for (let i = 0; i < count; i++) {
    ...
    cloudImg.src = `./images/background/cloud${Math.round(
...
function changePage() {
  ...
  switch (page.currentPage) {
    case 1:
      ...
      changeClass(
        [navAboutIcon, navProjectsIcon, navContactIcon],
        ["active-link"]
      );
